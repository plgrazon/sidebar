...
//      .catch((err) => console.log('err from post ', err))
  ...
  getResto(e) {
    axios('/api/restaurant', {params: { id: e }})
      .then(restaurant => {
        // delete restaurant.data.rows[0].id;
        // delete restaurant.data[0].rid;
        let data = restaurant.data.rows[0];
        // let hour = {data.}
        console.log('what is restaurant ', restaurant.data.rows[0]);
        this.setState({
          details: restaurant.data.rows[0],
          hours: restaurant.data.rows[0],
          misc: restaurant.data.rows[0]
        });
      })
      .catch(err => {
        console.log('failed /api/restaurant get ', err);
      })
  }
